...
case class KeyOp(keyWidth:BitCount) extends Component {
  val io = new Bundle {
    val lastRoundKey = in Bits(keyWidth)
  ...
  io.curKey := genRoundKey(io.lastRoundKey,io.roundNum)

  def driveFrom(lastRoundKey:Bits,roundNum:UInt):Bits = {
    io.lastRoundKey := lastRoundKey
...
case class AES_ENGINE(keyWidth:BitCount) extends Component {
  ...
  val CtrlState = new Area {
    ...
    when(io.engine.cmd.fire) {
      ...
      enginRdy := False
      rndCnt := 1
      currndKey := keyOp.driveFrom(io.engine.cmd.key.subdivideIn(32 bits).reverse.asBits,U(1,4 bits))
      dataBuf := rndFuc.driveFrom( io.engine.cmd.block ^ io.engine.cmd.key, currndKey,!lastRound)
    } .elsewhen(ongoing) {
      enginRdy := False
      currndKey := keyOp.driveFrom(lastKey.subdivideIn(32 bits).reverse.asBits,rndCnt + rndStep)
